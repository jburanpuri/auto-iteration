...
async function refresh() {
 try {
  ...
  if(batch&&['pending','summarizing'].includes(batch.status))$('#notice').textContent='Summarizing up to 20 reviews. Proposals will appear in Discord.';
...
$('#start').onclick=async()=>{
 ...
 $('#notice').classList.remove('error');$('#notice').textContent='Starting a summary of up to 20 reviews…';
 try {
