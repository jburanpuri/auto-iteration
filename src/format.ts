...
export function formatDiscordTask(task: Task, consoleUrl?: string): string {
  ...
  const trust = confidence ? `• **Non-spam confidence:** ${confidence.legitimacy === 'sample' ? 'Not assessed. Request a revised content assessment.' : `${confidence.legitimacy}. ${confidence.legitimacyReason}`}\n• **Issue confidence:** ${confidence.issue}. ${confidence.issueReason}` : '• **Confidence:** Not assessed in this older proposal. Request a revised plan.';
  ...
  if (plan.disposition === 'needs_clarification') {
    ...
    return [`**Needs clarification · v${plan.version}**`, title, '', plan.summary, trust, '',
      ...questions.map(question => `• ${question.replace(/^Clarify:\s*/i, '')}`),
  ...
  return [`**Proposed fix · v${plan.version}**`, title, '', `• **Issue:** ${plan.classification?.issue || title} (${task.signal?.reportCount || 1} reports)`, `• **Proposed fix:** ${plan.summary}`,
    `• **Code evidence:** ${finding || 'See the investigation details.'}`, trust,
    `• **Validation planned:** ${plan.acceptanceCriteria.slice(0, 2).join(' ')}`, '',
