...
export function formatDiscordTask(task: Task, consoleUrl?: string): string {
  ...
  if (plan.disposition === 'needs_clarification') {
    ...
    return [`**Needs clarification · v${plan.version}**`, title, '', brief(plan.summary, 400), '',
  ...
  return [`**Proposed fix · v${plan.version}**`, title, '', `**Fix:** ${brief(plan.summary, 380)}`,
    `**Code evidence:** ${brief(finding || 'See the investigation details.', 360)}`,
    `**Validation planned:** ${brief(plan.acceptanceCriteria.slice(0, 2).join(' '), 260)}`, '',
