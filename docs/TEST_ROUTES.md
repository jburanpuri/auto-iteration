...
| Route | Purpose |
| --- | --- |
| `/` | Reviews, issue summaries, and the manual workflow button |
| `/product/` | Company feedback form and intentionally buggy contact export |
...
Local routes:
- http://127.0.0.1:4348/ — company feedback form
...
1. Open `/product/`. Open “Export your contacts” and click Export contacts. The original empty-list export throws. Submit feedback; the subtle confirmation and unnecessary task polling are also intentional sample issues.
2. Open `/`. The 120 authored sample reviews are labeled as samples. Real submissions join the inbox. Click “Summarize & start workflow” and enter the demo code. Submissions are collected immediately; no timer is implemented.
